...
public class MemberController {
	...
	public String mailCheckGET(String email) throws Exception {
		...
		String content = "LookPlace를 방문해주셔서 감사합니다." + "<br><br>" + "인증번호는 " + checkNum + "입니다." + "<br>"
	...
	public String findPasswordAction(MemberDTO dto, Model model) {
		MemberDTO user = memberservice.findPassword(dto);
		
		if (user == null) {
			model.addAttribute("check", 1);
		} else {
			model.addAttribute("check", 0);
			model.addAttribute("updateid", user.getEmail());
		}
	...
	public String updatePasswordAction(@RequestParam(value ="updateid", defaultValue="", required=false) String id, MemberDTO dto) {
		dto.setEmail(id);
	...
	// 비밀번호 바꾸기할 경우 성공 페이지로 이동
	@RequestMapping(value = "checkpasswordview")
